...
class AbstractObjectsTest(unittest.TestCase):
    def test_text(self):
        ...
        segment_0 = txt.setCharacterStyle(0, 3, 5, characterStyle=AbstractCharacterStyle())

        self.assertEqual(len(txt.textSegments), 1)
        self.assertEqual(txt.textSegments[0].txtStr, "2017,")

        segment_1 = txt.setCharacterStyle(0, segment_0.endIndex, 8, characterStyle=AbstractCharacterStyle())
